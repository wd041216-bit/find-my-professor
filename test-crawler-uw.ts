...
import { ScrapingService } from './server/services/scraping';
import fs from 'fs';
...
async function testUWCrawler() {
  ...
  try {
    ...
    const task = await ScrapingService.triggerScrapingTask(universityName, majorName, degreeLevel);
    console.log(`✓ Scraping task triggered (Task ID: ${task.taskId}, Status: ${task.status})\n`);
...
async function generateReport(projects: any[]) {
