...
export const coverLetterRouter = router({
  /**
   * Generate a cover letter for a specific professor
   * Used in Match History page for liked professors
   */
  generateForProfessor: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const prompt = `You are an expert academic advisor helping a student write a compelling cover letter to express interest in research opportunities with a professor.

**CRITICAL INSTRUCTION: Write the entire cover letter in English only. The recipient is a professor who expects English communication.**
...
**Guidelines:**
1. Start with a strong opening that shows genuine interest in the professor's research
2. Highlight relevant skills and experiences from the student's activities
3. Explain why the student is a good fit for the professor's research area
4. Show knowledge of the professor's research interests
5. Express eagerness to contribute to the research
6. End with a clear call to action (requesting a meeting or discussion)
7. Use specific examples from the student's background
...
Return ONLY the cover letter text, without any additional commentary or meta-text. Do not include subject line or email headers.`;
      ...
      const result = await db.insert(coverLetters).values({
        userId,
        matchId: null, // No matchId for professor-based letters
        projectName: `Research with ${professor.name}`,
