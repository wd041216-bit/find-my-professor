...
// Princeton University所有93个主要专业（排除语言课程）
const DEPARTMENTS = [
  ...
  "Molecular Biology", "Music", "Music Composition", "Musicology",
  ...
  "Statistics and Machine Learning", "Sustainable Energy",
  "Technology and Society", "Theater and Music Theater",
  "Translation and Intercultural Communication", "Urban Studies", "Visual Arts"
];

async function callBuiltInLLM(prompt) {
...
If you cannot find professors for this department, return an empty array: []`;
  ...
  try {
    const responseText = await callBuiltInLLM(prompt);
...
async function main() {
  ...
  console.log('Princeton University Professor Data Collection (Built-in LLM)');
  console.log('='.repeat(70));
  ...
  for (let i = 0; i < DEPARTMENTS.length; i++) {
    ...
    try {
      ...
      console.log(`[Progress] ${i + 1}/${DEPARTMENTS.length} departments completed (${Math.round((i + 1) / DEPARTMENTS.length * 100)}%)`);
      console.log('');
