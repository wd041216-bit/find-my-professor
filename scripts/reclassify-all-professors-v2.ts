...
import { professors, researchFieldTagMapping } from "../drizzle/schema";
...
async function reclassifyAllProfessors() {
  ...
  const tagMappingRows = await db
    .select()
    .from(researchFieldTagMapping);
  
  const tagToField = new Map<string, string>();
  for (const row of tagMappingRows) {
    if (row.tag && row.researchFieldName) {
      tagToField.set(row.tag.toLowerCase().trim(), row.researchFieldName);
    }
  }
  ...
  for (const prof of professorsWithTags) {
    try {
      ...
      await db
        .update(professors)
        .set({ researchField: bestField })
        .where(sql`${professors.id} = ${prof.id}`);
