...
async function checkMatches() {
  ...
  try {
    ...
    const [matches] = await connection.query(`
      SELECT id, project_name, university, major, created_at 
    ...
    matches.forEach(m => {
      console.log(`  - ID: ${m.id}, Project: ${m.project_name}, University: ${m.university}, Major: ${m.major}, Created: ${m.created_at}`);
    });
    ...
    `);
    recent.forEach(m => {
      console.log(`  - ${m.university} / ${m.major}: ${m.project_name} (${m.created_at})`);
    });
